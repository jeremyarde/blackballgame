...
use reqwest::Client;
use reqwest_websocket::{websocket, Message, WebSocket};
...
fn App() -> Element {
    ...
    let test = use_coroutine(|mut rx: UnboundedReceiver<Message>| async move {
    ...
    let mut hand: Signal<Vec<Card>> = use_signal(|| vec![]);
    // Build cool things ✌️
    ...
    rsx! {
        ...
        div { class: "text-violet-800", "Hand: {hand:?}" }

        button { onclick: move |_| { test.send(Message::Text("Test message".into())) },
